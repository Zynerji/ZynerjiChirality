...
Beyond these initial findings, deep statistical analysis uncovered seven novel relationships: the CISS exponential decay law (Eq.~\ref{eq:ciss_decay}) provides a quantitative design rule showing that 90\% of spin-filter performance is achieved by CD~$\approx 0.44$; the 500-atom chirality cliff reveals a fundamental scale limitation of spectral detection; SG~211's extreme chirality amplification identifies rare-earth formates as prime candidates for magneto-chiral devices; the CD--band gap anticorrelation points to an intermediate CD sweet spot for photonic applications; enantiomorphic pair asymmetry warns against na\"ive database comparisons; the negative-CD enrichment reveals crystallization selection bias; and the element diversity--chirality correlation provides a practical screening heuristic.
